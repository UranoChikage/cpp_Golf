...
//Vector3オーバーロード版
XMMATRIX MatrixMath::Translation(const XMFLOAT3* v)
{
...
// Vector3 で3軸まとめて渡す版
//順番が関係するので注意
XMMATRIX MatrixMath::Rotation(const XMFLOAT3* anglesDeg)
{
	XMMATRIX rotX = RotationX(anglesDeg->x);
	XMMATRIX rotY = RotationY(anglesDeg->y);
	XMMATRIX rotZ = RotationZ(anglesDeg->z);
...
// Vector3 オーバーロード版
XMMATRIX MatrixMath::Scale(const XMFLOAT3* v)
{
...
XMMATRIX MatrixMath::RotationAxis(const XMFLOAT3& axis, const float angleDeg)
{
...
XMFLOAT3 MatrixMath::MultiplyPoint(const XMMATRIX& m, const XMFLOAT3& v)
{
	//w = 1で掛け算(位置の変換)
	XMVECTOR v4 = XMVectorSet(v.x, v.y, v.z, 1.0f);
	XMVECTOR result = XMVector3Transform(v4, m);
	return XMFLOAT3(XMVectorGetX(result), XMVectorGetY(result), XMVectorGetZ(result));
}
XMFLOAT3 MatrixMath::MultiplyVector(const XMMATRIX& m, const XMFLOAT3& v)
{
	//w = 0で掛け算(方向の変換：平行移動の影響を受けない )
	XMVECTOR v4 = XMVectorSet(v.x, v.y, v.z, 0.0f);
	XMVECTOR result = XMVector3Transform(v4, m);
	return XMFLOAT3(XMVectorGetX(result), XMVectorGetY(result), XMVectorGetZ(result));
}
