...
public:
	...
	*/
	...
	//Vector3オーバーロード版
	static XMMATRIX Translation(const XMFLOAT3* v);
	// ──────────────────────────
	...
	// Vector3 で3軸まとめて渡す版
	//順番が関係するので注意
	static XMMATRIX Rotation(const XMFLOAT3* anglesDeg);
	// ──────────────────────────────
	//  スケール行列 (Scale Matrix)
	...
	// Vector3 オーバーロード版
	static XMMATRIX Scale(const XMFLOAT3* v);
	//均等にスケールする版
	...
	// ──────────────────────────────────────────────
	static XMMATRIX RotationAxis(const XMFLOAT3& axis, const float angleDeg);
	//行列をベクターに適用する関数
	/// <summary>
	/// world座標の位置を変換したいときはこっち(MultiplyPoint)
	/// </summary>
	static XMFLOAT3 MultiplyPoint(const XMMATRIX& m, const XMFLOAT3& v);
	/// <summary>
	/// 方向を変換したいときはこっち(MultiplyVector)
	/// </summary>
	static XMFLOAT3 MultiplyVector(const XMMATRIX& m, const XMFLOAT3& v);
	// ──────────────────────────────
